...
class Clans(commands.Cog):
    ...
    async def clancreate(self, ctx, tag: str = None, *, name: str = None):
        ...
        try:
            conn.execute(
                "INSERT INTO clans (name, tag, leader_id, description) VALUES (?,?,?,?)",
                (name, tag, str(ctx.author.id), f"A proud Maratha clan led by {ctx.author.name}!")
            )
            clan_id = conn.execute("SELECT id FROM clans WHERE tag=?", (tag,)).fetchone()["id"]
            conn.execute(
                "UPDATE players SET clan_id=?, clan_role='leader', hon=hon-1000 WHERE user_id=?",
    ...
    async def claninfo(self, ctx, *, clan_name: str = None):
        ...
        if clan_name:
            clan = conn.execute(
                "SELECT * FROM clans WHERE name LIKE ? OR tag LIKE ?",
                (f"%{clan_name}%", f"%{clan_name.upper()}%")
            ).fetchone()
        elif player["clan_id"]:
            clan = conn.execute("SELECT * FROM clans WHERE id=?", (player["clan_id"],)).fetchone()
        else:
            conn.close()
        ...
        members = conn.execute(
            "SELECT username, clan_role FROM players WHERE clan_id=?", (clan["id"],)
        ).fetchall()
        conn.close()
    ...
    async def clanjoin(self, ctx, tag: str = None):
        ...
        conn = db.get_conn()
        clan = conn.execute("SELECT * FROM clans WHERE tag=?", (tag.upper(),)).fetchone()
        if not clan:
            conn.close()
        ...
        conn.execute(
            "UPDATE players SET clan_id=?, clan_role='member' WHERE user_id=?",
    ...
    async def clanleave(self, ctx):
        ...
        conn = db.get_conn()
        clan = conn.execute("SELECT name, tag FROM clans WHERE id=?", (player["clan_id"],)).fetchone()
        conn.execute("UPDATE players SET clan_id=NULL, clan_role=NULL WHERE user_id=?", (str(ctx.author.id),))
        conn.commit()
        conn.close()
    ...
    async def clandisband(self, ctx):
        ...
        clan = conn.execute("SELECT name, tag FROM clans WHERE id=?", (clan_id,)).fetchone()
        conn.execute("UPDATE players SET clan_id=NULL, clan_role=NULL WHERE clan_id=?", (clan_id,))
    ...
    async def clandonate(self, ctx, amount: int = None):
        ...
        conn = db.get_conn()
        conn.execute("UPDATE players SET hon=hon-? WHERE user_id=?", (amount, str(ctx.author.id)))
        conn.execute("UPDATE clans SET hon_bank=hon_bank+?, xp=xp+? WHERE id=?",
                     (amount, amount//10, player["clan_id"]))
        clan = conn.execute("SELECT name, tag, hon_bank FROM clans WHERE id=?", (player["clan_id"],)).fetchone()
        conn.commit()
        conn.close()
