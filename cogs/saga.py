...
class Saga(commands.Cog):
    ...
    async def saga(self, ctx, saga_num: int = None):
        ...
        # Select a saga
        conn = db.get_conn()
        saga = conn.execute("SELECT * FROM sagas WHERE id=?", (saga_num,)).fetchone()
        conn.close()
        ...
        # Set active saga
        conn = db.get_conn()
        conn.execute("UPDATE players SET active_saga=? WHERE user_id=?", (saga_num, str(ctx.author.id)))
        conn.commit()
        conn.close()
    ...
    async def mission(self, ctx, mission_num: int = None):
        ...
        conn = db.get_conn()
        saga    = conn.execute("SELECT * FROM sagas WHERE id=?", (saga_id,)).fetchone()
        missions = conn.execute("SELECT * FROM missions WHERE saga_id=? ORDER BY mission_num", (saga_id,)).fetchall()
        completed = {
            row["mission_num"]
            for row in conn.execute(
                "SELECT mission_num FROM player_missions WHERE user_id=? AND saga_id=? AND completed=1",
        ...
        mission = conn.execute(
            "SELECT * FROM missions WHERE saga_id=? AND mission_num=?", (saga_id, mission_num)
        ).fetchone()
        conn.close()
    ...
    async def _run_mission_battle(self, ctx, player, fighter, mission, saga):
        ...
        if player_hp > 0 and enemy_hp <= 0:
            ...
            conn = db.get_conn()
            conn.execute(
                "INSERT OR REPLACE INTO player_missions (user_id, saga_id, mission_num, completed) VALUES (?,?,?,1)",
                (str(ctx.author.id), saga["id"], mission["mission_num"])
            )
            conn.execute(
                "UPDATE players SET hon=hon+?, xp=xp+?, boss_keys=boss_keys+? WHERE user_id=?",
