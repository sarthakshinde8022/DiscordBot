...
def ensure_battle_stats(user_id):
    conn = db.get_conn()
    conn.execute("INSERT OR IGNORE INTO battle_stats (user_id) VALUES (?)", (str(user_id),))
    conn.commit()
    conn.close()
...
class Battle(commands.Cog):
    ...
    async def fight(self, ctx, opponent: discord.Member = None):
        ...
        if winner:
            ...
            conn = db.get_conn()
            conn.execute("UPDATE battle_stats SET pvp_wins=pvp_wins+1, total_dmg=total_dmg+? WHERE user_id=?", (wf["atk"], str(winner.id)))
            conn.execute("UPDATE battle_stats SET pvp_losses=pvp_losses+1 WHERE user_id=?", (str(loser.id),))
            conn.execute("UPDATE players SET hon=hon+?, xp=xp+? WHERE user_id=?", (hon_reward, xp_reward, str(winner.id)))
            conn.commit()
            conn.close()
    ...
    async def boss(self, ctx, *, boss_name: str = None):
        ...
        conn = db.get_conn()
        boss = conn.execute(
            "SELECT * FROM bosses WHERE name LIKE ?" if boss_name else "SELECT * FROM bosses ORDER BY RANDOM() LIMIT 1",
        ...
        # Deduct boss key
        conn = db.get_conn()
        conn.execute("UPDATE players SET boss_keys=boss_keys-1 WHERE user_id=?", (str(ctx.author.id),))
        conn.commit()
        conn.close()
        ...
        if won:
            conn = db.get_conn()
            conn.execute("UPDATE battle_stats SET boss_wins=boss_wins+1 WHERE user_id=?", (str(ctx.author.id),))
            conn.execute("UPDATE players SET hon=hon+?, xp=xp+? WHERE user_id=?", (boss["reward_hon"], boss["reward_xp"], str(ctx.author.id)))
            conn.commit()
            conn.close()
            result = discord.Embed(
    ...
    async def stats(self, ctx, member: discord.Member = None):
        ...
        conn = db.get_conn()
        bs = conn.execute("SELECT * FROM battle_stats WHERE user_id=?", (str(target.id),)).fetchone()
        conn.close()
