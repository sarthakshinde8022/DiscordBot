...
class Market(commands.Cog):
    ...
    async def _get_item_label(self, item_type, ref_id):
        ...
        if item_type == "char":
            row = conn.execute(
                "SELECT ch.name, ch.rarity FROM player_characters pc JOIN characters ch ON pc.char_id=ch.id WHERE pc.id=?",
                (ref_id,)
            ).fetchone()
            conn.close()
            if row:
            ...
            row = conn.execute(
                "SELECT i.name, i.rarity FROM player_items pi JOIN items i ON pi.item_id=i.id WHERE pi.id=?",
                (ref_id,)
            ).fetchone()
            conn.close()
            if row:
    ...
    async def mlist(self, ctx, item_type: str = None, item_id: int = None, price: int = None):
        ...
        if item_type == "char":
            row = conn.execute(
                "SELECT pc.id, ch.name, ch.rarity FROM player_characters pc JOIN characters ch ON pc.char_id=ch.id WHERE pc.id=? AND pc.user_id=?",
                (item_id, str(ctx.author.id))
            ).fetchone()
        else:
            row = conn.execute(
                "SELECT pi.id, i.name, i.rarity FROM player_items pi JOIN items i ON pi.item_id=i.id WHERE pi.id=? AND pi.user_id=?",
                (item_id, str(ctx.author.id))
            ).fetchone()
        ...
        conn.execute(
            "INSERT INTO market (seller_id, item_type, item_ref_id, price_hon) VALUES (?,?,?,?)",
    ...
    async def mbuy(self, ctx, listing_id: int = None):
        ...
        listing = conn.execute(
            "SELECT m.*, p.username as seller_name FROM market m JOIN players p ON m.seller_id=p.user_id WHERE m.id=? AND m.sold=0",
            (listing_id,)
        ).fetchone()
        ...
        if listing["item_type"] == "char":
            conn.execute(
                "UPDATE player_characters SET user_id=? WHERE id=?",
                (str(ctx.author.id), listing["item_ref_id"])
            )
        else:
            conn.execute(
                "UPDATE player_items SET user_id=?, is_equipped=0, equipped_to=NULL WHERE id=?",
                (str(ctx.author.id), listing["item_ref_id"])
            )

        # Transfer Hon
        conn.execute("UPDATE players SET hon=hon-? WHERE user_id=?", (listing["price_hon"], str(ctx.author.id)))
        conn.execute("UPDATE players SET hon=hon+? WHERE user_id=?", (listing["price_hon"], listing["seller_id"]))
    ...
    async def mremove(self, ctx, listing_id: int = None):
        ...
        conn = db.get_conn()
        listing = conn.execute(
            "SELECT * FROM market WHERE id=? AND seller_id=? AND sold=0",
    ...
    async def mylistings(self, ctx):
        ...
        conn = db.get_conn()
        listings = conn.execute(
            "SELECT * FROM market WHERE seller_id=? AND sold=0", (str(ctx.author.id),)
        ).fetchall()
        conn.close()
    ...
    async def trade(self, ctx, opponent: discord.Member = None, hon: int = 0):
        ...
        if hon > 0:
            conn = db.get_conn()
            conn.execute("UPDATE players SET hon=hon-? WHERE user_id=?", (hon, str(ctx.author.id)))
            conn.execute("UPDATE players SET hon=hon+? WHERE user_id=?", (hon, str(opponent.id)))
            conn.commit()
            conn.close()
