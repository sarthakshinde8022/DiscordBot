...
class Tower(commands.Cog):
    ...
    async def tower(self, ctx, difficulty: int = None):
        ...
        # Cooldown check
        conn = db.get_conn()
        attempt = conn.execute("SELECT last_attempt FROM tower_attempts WHERE user_id=?", (str(ctx.author.id),)).fetchone()
        conn.close()
        ...
        conn = db.get_conn()
        conn.execute(
            "INSERT OR REPLACE INTO tower_attempts (user_id, last_attempt) VALUES (?,?)",
        ...
        if won_all:
            ...
            conn.commit()
            new_keys = conn.execute("SELECT boss_keys FROM players WHERE user_id=?", (str(ctx.author.id),)).fetchone()["boss_keys"]
            conn.close()
